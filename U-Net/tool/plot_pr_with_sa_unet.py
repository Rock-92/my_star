...
for path in (PROJECT_TOOL_DIR, UNET_DIR, UNET_TOOL_DIR):
...
def load_sa_unet_class():
    path = ROOT / "SA-U-Net" / "train_sa_unet_softmask.py"
    spec = importlib.util.spec_from_file_location("sa_unet_train", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Cannot load SA-U-Net module from {path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.SAUNet256
...
def mask_centroids_legacy(prob: np.ndarray, threshold: float, min_area: int = 2, exclude_border: int = 8) -> np.ndarray:
    ...
    for label_id, slc in enumerate(ndi.find_objects(labels), start=1):
        ...
        weights = np.where(local, np.clip(local_prob - float(threshold), 1e-4, None), 0.0)
        total = float(weights.sum())
        y0 = slc[0].start
        x0 = slc[1].start
        yy, xx = np.mgrid[y0 : slc[0].stop, x0 : slc[1].stop].astype(np.float64)
        if total > 0:
            cy = float((yy * weights).sum() / total)
            cx = float((xx * weights).sum() / total)
        else:
            coords = np.argwhere(local)
            cy = float(y0 + coords[:, 0].mean())
            cx = float(x0 + coords[:, 1].mean())
        points.append((float(local_prob[local].max()), cy, cx))
...
def plot_split(df: pd.DataFrame, split_name: str, out: Path) -> None:
    ...
    line_width = 0.45
    marker_size = line_width * 4.0
    styles = (
        ("daofind", "red", "o", "daofind-like"),
        ("unet_legacy", "gray", "s", "U-Net old CC"),
        ("unet_peak", "green", "^", "U-Net peak-split"),
        ("sa_unet_peak", "blue", "D", "SA-U-Net peak-split"),
    )
    for family, color, marker, label in styles:
        ...
        ax.plot(
            ...
            markeredgewidth=line_width * 1.2,
            label=label,
            alpha=0.92,
        )
...
def load_unet(checkpoint: Path, device: torch.device) -> UNet256:
...
def load_sa_unet(checkpoint: Path, device: torch.device) -> torch.nn.Module:
    ckpt = torch.load(checkpoint, map_location=device)
    config = ckpt.get("config", {})
    base = int(config.get("base_channels", 32))
    neighborhood_sizes = tuple(int(v) for v in config.get("neighborhood_sizes", [5, 7, 9, 9]))
    max_heads = int(config.get("max_heads", 4))
    cls = load_sa_unet_class()
    model = cls(base=base, neighborhood_sizes=neighborhood_sizes, max_heads=max_heads).to(device)
    model.load_state_dict(ckpt["model"])
    model.eval()
    return model
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot PR curves for daofind-like, U-Net legacy/new mask postprocess, and SA-U-Net.")
    ...
    parser.add_argument("--unet-checkpoint", type=Path, default=Path("U-Net/runs/unet_softmask_10ep/best.pt"))
    parser.add_argument("--sa-checkpoint", type=Path, default=Path("SA-U-Net/runs/sa_unet_softmask_10ep/best.pt"))
    parser.add_argument("--output", type=Path, default=Path("test/pr_unet_sa_peak_split"))
    parser.add_argument("--model-thresholds", default="0.08,0.10,0.12,0.15,0.18,0.20,0.22,0.25,0.28,0.30,0.32,0.35,0.38,0.40,0.42,0.45,0.48,0.50,0.52,0.55,0.58,0.60,0.65,0.70,0.75,0.80,0.85,0.90")
    parser.add_argument("--daofind-sigmas", default="3.00,3.25,3.50,3.75,4.00,4.25,4.50,4.75,5.00,5.25,5.50,5.75,6.00,6.25,6.50,7.00")
...
def main() -> None:
    ...
    if "feature_split" not in selected.columns:
        raise SystemExit("selected_samples.csv needs a feature_split column.")
    selected = selected[selected["feature_split"].isin(["train", "val"])].copy().reset_index(drop=True)

    unet = load_unet((ROOT / args.unet_checkpoint).resolve(), device)
    sa_unet = load_sa_unet((ROOT / args.sa_checkpoint).resolve(), device)
    data_model = (ROOT / args.data_model).resolve()
    data_dir = (ROOT / args.data_dir).resolve()
    gaia_dir = (ROOT / args.gaia_dir).resolve()
    thresholds = parse_csv_floats(args.model_thresholds)
    ...
    for i, row in selected.iterrows():
        ...
        image = soft["image"].astype(np.float32)

        unet_prob_path = cache / f"{sample_id}_unet_prob.npy"
        if unet_prob_path.exists():
            unet_prob = np.load(unet_prob_path).astype(np.float32)
        else:
            unet_prob = infer_full(unet, image, args.patch_size, args.stride, device)
            np.save(unet_prob_path, unet_prob.astype(np.float32))

        sa_prob_path = cache / f"{sample_id}_sa_unet_prob.npy"
        if sa_prob_path.exists():
            sa_prob = np.load(sa_prob_path).astype(np.float32)
        else:
            sa_prob = infer_full(sa_unet, image, args.patch_size, args.stride, device)
            np.save(sa_prob_path, sa_prob.astype(np.float32))

        for thr in thresholds:
            for family, method, prob, detector in (
                ("unet_legacy", "U-Net old CC", unet_prob, mask_centroids_legacy),
                ("unet_peak", "U-Net peak-split", unet_prob, mask_centroids),
                ("sa_unet_peak", "SA-U-Net peak-split", sa_prob, mask_centroids),
            ):
                pred = detector(prob, threshold=thr, min_area=2, exclude_border=8)
                met, _ = metrics(f"{family}_thr{thr:g}", pred, target, args)
                met.update({"split_name": split_name, "family": family, "method": method, "param": float(thr)})
                rows.append(met)

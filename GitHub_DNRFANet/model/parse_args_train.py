...
def parse_args():
    ...
    # choose model
    parser.add_argument('--model', type=str, default='DNRFANet', help='CDNNet, DNANet, StarNet, UNet, DNRFANet, AGPCNet, HCFNet, SeRankDet')
    ...
    parser.add_argument('--scale_method', type=str, default='deconv', help='deconv, biinterp, nearest')
    parser.add_argument('--deep_supervision', type=str, default='True', help='True or False')

    # data and pre-process
    parser.add_argument('--dataset', type=str, default='fits_softCircle_modified', help='dataset name: fits_softCircle_modified')
    ...
    parser.add_argument('--preprocess', type=str, default='hierarchy', help='hierarchy, minmax, meanstd, sigmacut')
    parser.add_argument('--mask_type', type=str, default='soft', help='soft, hard, SLS')
    parser.add_argument('--workers', type=int, default=2, metavar='N', help='dataloader threads,2')
    parser.add_argument('--in_channels', type=int, default=3, help='according to pre-process method')
    ...
    parser.add_argument('--epochs', type=int, default=100, metavar='N', help='number of epochs to train (default: 110)')
    ...
    parser.add_argument('--scheduler_dethead', default='CosineAnnealingLR', choices=['CosineAnnealingLR', 'ReduceLROnPlateau'])
    parser.add_argument('--lr', type=float, default=0.0001, metavar='LR', help='learning rate (default: 0.1, 0.05)')
    parser.add_argument('--loss_func', type=str, default='SIoU+SFL', help='for hardmask: IoU; for softmask: SIoU, SFL, SIoU+SFL, SLS')
    ...
    parser.add_argument('--model_dir', type=str, default ='D:/DNRFA-Net/result/fits_softCircle_modified_DNRFANet_20250512/DNRFANet_fits_softCircle_modified_best_head.pth.tar', help='model path')
    ## parse args
    ...
    if args.preprocess.lower() in ['minmax','meanstd','sigmacut'] and args.in_channels!=1:
        args.in_channels = 1
        print('in_channels is set to 1, under {} preprocess training mode'.format(args.preprocess.lower()))
    if args.mask_type.lower()=='hard' and args.loss_func!='IoU':
        args.loss_func = 'IoU'
        print('loss function is set to IoU-loss, under hard-mask training mode')

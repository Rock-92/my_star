...
class Trainer(object):
    def __init__(self, args):
        ...
        testset = TestSetLoaderFits(dataset_dir, img_id=self.val_img_ids, preprocess=args.preprocess,  # args.preprocess
                                    base_size=args.base_size, crop_size=args.crop_size, transform=input_transform, 
                                    suffix=args.suffix, label_mode=args.label_mode)
        ...
        elif args.model.lower() == 'dnrfanet':
            ARFAM, DNRFANet = get_model(args.model)
            model = DNRFANet(num_classes=1, input_channels=args.in_channels, block=ARFAM, num_blocks=num_blocks,
                              nb_filter=nb_filter, netdepth=args.netdepth, scale_method=args.scale_method,
                              deep_supervision=args.deep_supervision)
        elif args.model.lower() == 'agpcnet':
            AGPCNet = get_model(args.model)
            model = AGPCNet(backbone='resnet18')
        elif args.model.lower() == 'hcfnet':
            HCFnet = get_model(args.model)
            model = HCFnet()
        elif args.model.lower() == 'serankdet':
            SeRankDet = get_model(args.model)
            model = SeRankDet()
        else:
        ...
        self.best_f1 = 0
        self.best_recall = 0
        self.test_fitsimg = fits.getdata("D:/DNRFA-Net/dataset/fitstrans/fitsdata/4/20200821152001999_7824_824122_LVT04.fits")
    ...
    def training(self, epoch):
        ...
        tbar.set_description(f'Epoch {epoch}, Preprocess: {self.args.preprocess}')
            ...
            if args.deep_supervision == 'True':
                ...
                for pred in preds:
                    if self.args.mask_type == 'hard':
                        if self.args.loss_func =='IoU':
                            loss += smoothiouLoss(pred, labels)
                        else:
                            raise Exception('wrong loss function for hardmask')
                    elif self.args.mask_type == 'soft':
                        if self.args.loss_func =='SIoU':
                            loss += siouLoss(pred, labels)
                        elif self.args.loss_func =='SFL':
                            loss += sflLoss(pred, labels) / 100    # for real: /100; for simu: /800
                        elif self.args.loss_func =='SIoU+SFL':
                            ''' 1. SIoU-loss '''
                            loss += siouLoss(pred, labels)
                            ''' 2. SFL-loss '''
                            loss += sflLoss(pred, labels) / 100    # for real: /100; for simu: /800
                        elif self.args.loss_func =='SIoU+SFL+SLS':
                            ''' 1. SIoU-loss '''
                            loss += siouLoss(pred, labels)
                            ''' 2. SFL-loss '''
                            loss += sflLoss(pred, labels) / 100
                            ''' 3. SLS '''
                            loss += SLSIoULoss(pred, labels, self.warm_epoch, epoch, with_shape=True)
                        elif self.args.loss_func =='SIoU+SFL+BCE':
                            ''' 1. SIoU-loss '''
                            loss += siouLoss(pred, labels)
                            ''' 2. SFL-loss '''
                            loss += sflLoss(pred, labels) / 100
                        ...
                        elif self.args.loss_func =='SIoU+SFL+SL':
                            ''' 1. SIoU-loss '''
                            loss += siouLoss(pred, labels)
                            ''' 2. SFL-loss '''
                            loss += sflLoss(pred, labels) / 100
                            ''' 3. Scale-loss '''
                            loss += SLoss(pred, labels)
                        else:
                            raise Exception('wrong loss function for softmask')
                    else:
                        if self.args.loss_func == 'SLS':
                            loss += SLSIoULoss(pred, labels, self.warm_epoch, epoch, with_shape=True)
                        else:
                            raise Exception('wrong loss function for softmask')
                loss /= len(preds)
    ...
    def testing(self, epoch):
        ...
        self.model.eval()
        with torch.no_grad():
            tbar.set_description(f'Testing Epoch {epoch}, Preprocess: {self.args.preprocess}')
            ...
            fname = "D:/DNRFA-Net/dataset/fitstrans/fitsdata/4/20200821152001999_7824_824122_LVT04.IPD"
            ...
            if precision != 0 and recall != 0:
                ...
                f1 = 0
                faRate = 0
            nme = self.calculate_normalized_mean_error(df_pred, df_gt, thres=1.5)
            print('Epoch:{}, precision:{}, recall:{}, f1:{}, faRate:{}, NME:{}'.format(epoch, precision, recall, f1, faRate, nme))

            # save result
            save_good_dir = 'result/' + self.save_dir + '/' + self.save_prefix + '_good_precision_recall.log'
            with open(save_good_dir, 'a') as f:
                f.write('Epoch:{}, precision:{}, recall:{}, f1:{}, faRate:{}, NME:{}\n'.format(epoch, precision, recall, f1, faRate, nme))
